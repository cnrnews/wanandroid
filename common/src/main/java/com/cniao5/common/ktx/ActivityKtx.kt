 ...
 * 设置动画
 */
fun Activity.setTransition(inTransition:Int,outTransiton:Int){
    val inTransition: Transition = TransitionInflater.from(this)
        .inflateTransition(inTransition)
    val outTransition: Transition = TransitionInflater.from(this)
        .inflateTransition(outTransiton)
    window.enterTransition = inTransition
    window.exitTransition = outTransition
}
