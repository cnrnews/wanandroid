...
inline fun <reified T> BaseResponse.toEntity(): T? {
    if (data == null) {
        LogUtils.e("server Response Json Ok,But data==null,$errorCode,$errorMsg")
        return null
    }
    ...
    return kotlin.runCatching {
        ...
        // }
        LogUtils.d("T:" + T::class.java)
        LogUtils.d("data:" + data.toString())

        fromJson(Gson().toJson(data),T::class.java)
    }.onFailure { e ->
...
inline fun BaseResponse.onBizError(crossinline block: (code: Int, message: String?) -> Unit): BaseResponse {
    ...
    if (errorCode != BaseResponse.SERVER_CODE_SUCCESS1 && errorCode != BaseResponse.SERVER_CODE_SUCCESS) { //同时不等于1和1001的时候执行if
        block.invoke(errorCode, errorMsg ?: "Error Message Null") //返回错误码和错误信息
    }
    return this
}
...
inline fun <reified T> BaseResponse.onBizOK(crossinline action: (code: Int, data: T?, message: String?) -> Unit): BaseResponse {
    ...
    if (errorCode == BaseResponse.SERVER_CODE_SUCCESS || errorCode == BaseResponse.SERVER_CODE_SUCCESS1) { //code == 1001或code == 1,成功
        action.invoke(errorCode, this.toEntity<T>(), errorMsg) //返回成功码和解密之后的序列化对象
    }
    return this
}


// region DataResult 扩展
